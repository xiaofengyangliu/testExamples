...
public class CopyOfGithubRepoPageProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		if (url.indexOf(".html") != -1) {
			...
			for (int i = 0; i < ss.length; i++) {//ss.length
				...
				int trueIndex = one.indexOf("六百六十七");
				if (trueIndex > 0) {
					String result = one.substring(startIndex + 1, endIndex);
					System.out.println(result);
					results.add("http://www.biquke.com/bq/9/9118/" + result
							+ ".html");
				}
	...
	public static void main(String[] args) {
		...
		Spider.create(new CopyOfGithubRepoPageProcessor())
		//http://www.biquke.com/bq/3/3714
				.addUrl("http://www.biquke.com/bq/3/3714")
				// 开启5个线程抓取
				.thread(5)
				// 启动爬虫
				.run();
